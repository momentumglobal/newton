...
**⚠️ New deploy step: bump `CONFIG.APP_BUILD` whenever you change anything under `js/`.** Every cache key embeds the build stamp, and api.js discards entries from any other build on load, so bumping it makes the next page load start from clean data. Change it in the same commit as the code, and bump `SW_VERSION` in `sw.js` alongside it — the two are separate on purpose (a service worker cannot read `config.js`) and neither substitutes for the other.

Forgetting costs up to 10 minutes of stale reference data for anyone mid-session, not a whole session: `_ssGet()` checks the TTL independently of the stamp, so entries expire on their own either way. The case that genuinely matters is a **shape** change — above all a `FIELD_ALIASES` edit, since aliases are not part of the cache key and normalisation happens before caching, so old-shape rows get read back under the wrong field names (the N-073 class: a silently blanked field, no error). A `LIST_FIELDS` change is self-correcting because the `$select` string is part of the key. Bump on every `js/` change anyway — it is one character and saves the judgement call.
