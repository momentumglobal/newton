...
**Delta sync (N-186), piloted on WeeklyActivity.** `getItems()` no longer re-fetches and re-caches an entire list on every cache miss for an unfiltered read. It calls SharePoint's `$delta` endpoint instead, keeps a running baseline array in `sessionStorage` alongside the sync token, and merges only the rows Graph reports changed — including deletions via the `@removed` marker — rather than replacing the array. Bootstraps once (no stored token yet), then every subsequent miss is a genuinely incremental pull. Falls back to a fresh bootstrap, once, on any incremental failure (a stale/expired token, a corrupted entry) rather than looping. Only engages on an unfiltered read (`filter === ""`), because SharePoint list-item delta queries do not support `$filter` — a filtered read against a delta-enrolled list is completely unaffected and falls through to the existing tier-1/tier-2/paginated path exactly as before.

**Rolled out to Placements (N-187) with a one-line config change, engine untouched.** N-186 built the engine entirely list-agnostic — no list name appears anywhere in `api.js`, only in `CONFIG.DELTA.enrolledLists` — so enrolling a second list needed nothing but that array. The rollout is small only because two defaults from earlier tickets already made the common case unfiltered: `CONFIG.DATE_WINDOW_DEFAULT_WEEKS` and `CONFIG.PLACEMENTS_DEFAULT_WEEKS` (N-158) both default to "All time" (0), so the main Activity and Placements pages' default reads carry no `$filter` and are delta-eligible out of the box. A full call-site catalogue — which reads benefit, which stay permanently on the paged path because they carry a project/role-scoped filter — lives as a comment in `config.js` beside `CONFIG.DELTA`, not just in the ticket.

**$batch request coalescing (N-188).** Pages that already parallelise several independent reads with `Promise.all()` (Company Dashboard, Report Builder) still cost one Graph round trip per read, each with its own token acquisition. `graphRequest()` now transparently queues a non-elevated GET call and flushes the queue on a macrotask boundary (`setTimeout(…, 0)`) — not a microtask, deliberately: concurrent wrapper functions (`getProjects`, `getWeeklyActivity`, …) cross a different number of internal cache-check awaits before any of them reach `graphRequest`, so a microtask flush would fire before most of the cohort arrives, batching nothing. A macrotask boundary waits for the whole microtask queue — every pending cache-check across every concurrent caller — to drain first. Everything else (writes, elevated calls, a lone GET with nothing else in flight) still dispatches immediately, solo. A 429/503 on one sub-request inside a batch falls back to a solo, retryable call for just that item, reusing the existing N-082 retry loop rather than a second implementation of it.

**Config:** `CONFIG.DELTA` (`enabled`, `enrolledLists`) and `CONFIG.BATCH` (`enabled`, `maxSubRequests` — Graph's own documented per-`$batch` limit, not a tuning knob). No SharePoint change. No new files — all three tickets live in `api.js` (request layer) and `config.js` (enrolment / kill switches) only.
...
**`CONFIG.APP_BUILD` is stamped automatically on deploy (N-220).** The `deploy` job writes a fresh value derived from the commit SHA into `js/config.js` at deploy time, in the runner's checkout only — never committed to the repo, no manual step. The dead `SW_VERSION` constant in `sw.js` (declared but never read) was removed in the same change.

The stamp exists because every cache key embeds it, and api.js discards entries from any other build on load. The case that genuinely matters is a **shape** change — above all a `FIELD_ALIASES` edit, since aliases are not part of the cache key and normalisation happens before caching, so old-shape rows get read back under the wrong field names (the N-073 class: a silently blanked field, no error). A `LIST_FIELDS` change is self-correcting because the `$select` string is part of the key.
