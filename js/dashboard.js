...
// Returns { start: Date, end: Date } for a given detail period key
...
// WeeklyActivity rows use Year + WeekNumber — derive the Friday (week ending) date for range comparison
...
function renderPipelineActivityTable(acts, roles, period) {
  ...
  // Only roles with activity in the period
  const byRole = {};
  filtered.forEach(a => {
...
// ── State ─────────────────────────────────────────────────────────────
let _companyPeriod       = 'quarter'; // KPI ribbon
let _companyDetailPeriod = 'this_month'; // Detail panels
...
// ── Activity by Talent Partner (company-wide, with Project column) ────
