...
// ── People module: Employee Tracker ──────────────────────────
let _peopleTab        = 'employees';  // 'employees' | 'assignments'
let _showInactive     = false;
let _assignmentFilter = { employee: '', customer: '', year: String(new Date().getFullYear()), billed: '' };
...
function _peopleTabBar() {
  ...
  return `<div class='filter-group' style='margin-bottom:16px'>
    ${tabs.map(t => `<button class='btn-filter${_peopleTab === t.key ? ' active' : ''}'
      onclick='_switchPeopleTab("${t.key}")'>${t.label}</button>`).join('')}
  </div>`;
}
...
async function renderEmployeesTab() {
  ...
  const people  = await getPeople(!_showInactive);  // false = include inactive
...
async function renderAssignmentsTab() {
  ...
  // Build dropdown option lists from live data
    ...
    <div class='project-filter-bar' style='display:flex;gap:16px;flex-wrap:wrap;margin-bottom:12px'>
      <div class='form-group' style='min-width:160px'>
        ...
        <select onchange="_setAssignmentFilter('employee',this.value)">
          ${opts(employees, _assignmentFilter.employee, 'All employees')}</select>
      </div>
      <div class='form-group' style='min-width:140px'>
        <label>Customer</label>
        <select onchange="_setAssignmentFilter('customer',this.value)">
          ${opts(customers, _assignmentFilter.customer, 'All customers')}</select>
      </div>
      <div class='form-group' style='min-width:100px'>
        <label>Year</label>
        <select onchange="_setAssignmentFilter('year',this.value)">
          ${opts(years, _assignmentFilter.year, 'All years')}</select>
      </div>
      <div class='form-group' style='min-width:100px'>
        <label>Billed</label>
