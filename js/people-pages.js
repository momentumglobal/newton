...
async function _syncBenchAssignments() {
  ...
  try {
    ...
    // Existing bench records — identified by Customer = Unassigned and Billed = No
    const existingBench = assignments.filter(a =>
      (!a.Customer || a.Customer === 'Unassigned') && a.Billed === 'No'
    );
    console.log('[BenchSync] existingBench:', existingBench.map(b => b.EmployeeName + ' ' + b.StartDate));
    console.log('[BenchSync] all AutoGenerated values:', assignments.filter(a => a.EmployeeName === 'Juan Drotskie').map(b => ({id: b.id, customer: b.Customer, autoGen: b.AutoGenerated, type: typeof b.AutoGenerated})));
