      ...
      <td class='salary-cell'>
        <span class='salary-masked' id='sal-masked-${p.id}' style='display:${_salariesRevealed ? "none" : "inline"}'>
          ...
          <button class='btn-padlock' title='Reveal salary' onclick='_revealSalary(${p.id})'
            style='background:none;border:none;cursor:pointer;padding:0 4px;color:var(--c-gray-500)'>🔒</button>
        </span>
  ...
  const rows = filtered.map(a => `
    <tr>
      ...
      <td><span class='badge badge-${a.Billed==="Yes"?"active":"inactive"}'>${escHtml(a.Billed)}</span>${
        isForecastAssignment(a) ? ` <span class='badge' style='background:var(--c-warn-bg-soft);color:var(--c-warn-text)'>Forecast</span>` : ''}</td>
      ${canEdit ? `<td><div class='row-actions'>
        ...
        ` · <a href='#' style='color:var(--c-danger)' onclick='_deleteAssignment(${a.id})'>Delete</a>`}
      </div></td>` : ''}
    </tr>`).join('');
  ...
  main.innerHTML = `
    <div class='page-header'>
      ...
      <div style='display:flex;align-items:center;gap:12px'>
        ...
        <span id='bench-sync-time' style='font-size:12px;color:var(--c-gray-500)'></span>` : ''}
      </div>
    </div>
    ${_peopleTabBar()}
    ${filterBar}
