...
// Same contract as pages.js's row builders -- opts.pending renders a
// row-pending class + data-pending-id and drops anything that depends on
// a real id. Uses pendingRowId() from utils.js; not redefined here.
...
// ── Bench Sync ────────────────────────────────────────────────
// Pure gap computation: given already-fetched People/Assignments arrays (and
// an injectable `today` for deterministic testing), returns the bench
// records to create/delete. No DOM, no API calls — see _syncBenchAssignments
// below for the async wrapper that fetches, executes the changes, and
// refreshes the UI.
//
...
// every sync run.
function _sbaComputeChanges(people, assignments, today = new Date()) {
...
async function _syncBenchAssignments() {
  ...
  try {
    ...
    const { toCreate, toDelete } = _sbaComputeChanges(people, assignments);
